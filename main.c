...
//Copia do pacote corrente usado pra reenvio
struct pkt a_cur_pkt;
struct pkt b_cur_pkt;
...
struct pkt checksum_gen(packet)
  struct pkt packet;
{
    int i, check = 0;
    check += packet.seqnum;
    //printf("\t+ seqnum: %d", check);
    check += packet.acknum;
    //printf("\t+ acknum: %d", check);
    for (i=0; i<20; i++){
  ...
  struct msg message;
{
    int i;
    //printf("\nA_output\n");
    ///Caso não tenha enviar a mensagem
    printf("\nA - Solicitação de envio mensagem: \"");
    for (i=0; i<20; i++)
        printf("%c",message.data[i]);
     printf("\"\n");
     ///Checar se há mensagens enviadas ainda não confirmadas
    if(a_last_pkt_acked == 0){
        printf("\nA - Pacote %d ainda não foi confirmado!", a_current_pkt_id);
        printf("\nA - Abortando a mensagem atual...\n");
    } else {
        a_last_pkt_acked = 0;
        ///Criar pacote
        printf("\nA - Criando pacote:\n");
        ...
        a_cur_pkt.seqnum = a_current_pkt_id;
        a_cur_pkt.acknum = 0;
        a_cur_pkt.checksum = 0;
        printf("\tSequencia: %d", a_cur_pkt.seqnum);
        printf("\tAcknum: %d", a_cur_pkt.acknum);
        printf("\tChecksum: %d", a_cur_pkt.checksum);
        printf("\tDados:\"");
        for (i=0; i<20; i++){
            a_cur_pkt.payload[i] = message.data[i];
            printf("%c", a_cur_pkt.payload[i]);
        }
        printf("\"\n");
        ///Preparar o checksum
        printf("\nA - Preparando o checksum");
        a_cur_pkt = checksum_gen(a_cur_pkt);
        ///ligar o timer
        printf("\nA - Iniciando timer");
  ...
  struct pkt packet;
{
    ...
    if(packet.acknum == 1){//ACK Pacote de confirmação
        if(checksum_dec(packet) == 1){
            printf("\nA - Pacote de confirmação ACK corrompido");
            printf("\nA - Ignorando o pacote\n");
        }
        else{
        ...
        if(checksum_dec(packet) == 1){
            printf("\nA - Pacote de confirmação NACK corrompido");
            printf("\nA - Ignorando o pacote\n");
        }
        else{
            printf("\nA - Pacote de confirmação NACK de recebimento mal sucedido");
            printf("\nA - Parando o timer do pacote %d", packet.seqnum);
            stoptimer(0);
            printf("\nA - Reiniciando timer");
            starttimer(0, 20.0); //leva 5 pra mandar e mais 5 pro retorno do ACK
        ...
        if(checksum_dec(packet) == 1){
            ...
            packet.acknum = 2;
            packet = checksum_gen(packet);
            tolayer3(0,packet);
        }
        else {
            ///Envia o ACK para B
            printf("\nA - O pacote está Ok!");
            printf("\nA - Enviando mensagem de confirmação para B");
            packet.acknum = 1;
            packet = checksum_gen(packet);
            tolayer3(0,packet);
...
A_timerinterrupt()
{
    printf("\nA - Tempo de espera pelo ACK do pacote %d expirou!", a_cur_pkt.seqnum);
    printf("\nA - Reiniciando timer");
    starttimer(0, 20.0); //leva 5 pra mandar e mais 5 pro retorno do ACK
    printf("\nA - Reenviando pacote atual para B\n");
    tolayer3(0,a_cur_pkt);
...
B_input(packet)  struct pkt packet;
{
    ...
    printf("\tSequencia: %d", packet.seqnum);
    printf("\tAcknum: %d", packet.acknum);
    printf("\tChecksum: %d", packet.checksum);
    printf("\tDados:\"");
    for (i=0; i<20; i++)
    ...
    if(packet.acknum == 1){//ACK Pacote de confirmacao
        if(checksum_dec(packet) == 1){
                printf("\nB - Pacote de confirmação ACK corrompido");
                printf("\nB - Ignorando o pacote\n");
        }
        else{
        ...
        if(checksum_dec(packet) == 1){
            printf("\nB - Pacote de confirmação NACK corrompido");
            printf("\nB - Ignorando o pacote\n");
        }
        else{
            printf("\nB - Pacote de confirmação NACK de recebimento mal sucedido");
            printf("\nB - Parando o timer do pacote %d", packet.seqnum);
            stoptimer(1);
            printf("\nB - Reiniciando timer");
        ...
        if(checksum_dec(packet) == 1){
            ...
            ///Envia o ACK para A
            printf("\nB - O pacote está Ok!");
            printf("\nB - Enviando mensagem de confirmação para A");
            packet.acknum = 1;
            packet = checksum_gen(packet);
            tolayer3(1,packet);
            ///Envia os dados para a aplicação
